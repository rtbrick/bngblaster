...
isis_pdu_load(isis_pdu_s *pdu, uint8_t *buf, uint16_t len)
{
    ...
    hdr_len = *ISIS_PDU_OFFSET(pdu, ISIS_OFFSET_HDR_LEN);
    if(hdr_len > len) {
        return DECODE_ERROR;
    }
    pdu->tlv_offset = hdr_len;
    system_id_len = *ISIS_PDU_OFFSET(pdu, ISIS_OFFSET_HDR_SYSTEM_ID_LEN);
    ...
    switch (pdu->pdu_type) {
        ...
        case ISIS_PDU_L2_CSNP:
            ...
            pdu_len = be16toh(*(uint16_t*)ISIS_PDU_OFFSET(pdu, ISIS_OFFSET_CSNP_LEN));
            break;
        ...
        case ISIS_PDU_L2_PSNP:
            ...
            pdu_len = be16toh(*(uint16_t*)ISIS_PDU_OFFSET(pdu, ISIS_OFFSET_PSNP_LEN));
            break;
        ...
        case ISIS_PDU_L2_LSP:
            ...
            pdu_len = be16toh(*(uint16_t*)ISIS_PDU_OFFSET(pdu, ISIS_OFFSET_LSP_LEN));
            break;
        default:
            return DECODE_ERROR;
    }
    ...
    ISIS_PDU_CURSOR_SET(pdu, hdr_len);
    while(ISIS_PDU_CURSOR_LEN(pdu) >= sizeof(isis_tlv_s)) {
        tlv = (isis_tlv_s*)ISIS_PDU_CURSOR(pdu);
        ISIS_PDU_CURSOR_INC(pdu, sizeof(isis_tlv_s));
        if(tlv->len > ISIS_PDU_CURSOR_LEN(pdu)) {
            return DECODE_ERROR;
        }
        switch(tlv->type) {
            case ISIS_TLV_AUTH:
                ...
                pdu->auth_type = *ISIS_PDU_CURSOR(pdu);
                pdu->auth_data_len = tlv->len - sizeof(pdu->auth_type);
                pdu->auth_data_offset = ISIS_PDU_CURSOR_GET(pdu) + sizeof(pdu->auth_type);
                break;                
            default:
                break;
        }
        ISIS_PDU_CURSOR_INC(pdu, tlv->len);
    }
...
isis_pdu_update_len(isis_pdu_s *pdu)
{
    switch (pdu->pdu_type) {
        case ISIS_PDU_P2P_HELLO:
            ...
            *ISIS_PDU_OFFSET(pdu, ISIS_OFFSET_HDR_LEN) = ISIS_HDR_LEN_COMMON+ISIS_HDR_LEN_P2P_HELLO;
            *(uint16_t*)ISIS_PDU_OFFSET(pdu, ISIS_OFFSET_HELLO_LEN) = htobe16(pdu->pdu_len);
            break;
        case ISIS_PDU_L1_HELLO:
        case ISIS_PDU_L2_HELLO:
            pdu->tlv_offset = ISIS_HDR_LEN_COMMON+ISIS_HDR_LEN_HELLO;
            *ISIS_PDU_OFFSET(pdu, ISIS_OFFSET_HDR_LEN) = ISIS_HDR_LEN_COMMON+ISIS_HDR_LEN_HELLO;
            *(uint16_t*)ISIS_PDU_OFFSET(pdu, ISIS_OFFSET_HELLO_LEN) = htobe16(pdu->pdu_len);
            break;
        case ISIS_PDU_L1_CSNP:
        case ISIS_PDU_L2_CSNP:
            pdu->tlv_offset = ISIS_HDR_LEN_COMMON+ISIS_HDR_LEN_CSNP;
            *ISIS_PDU_OFFSET(pdu, ISIS_OFFSET_HDR_LEN) = pdu->tlv_offset;
            *(uint16_t*)ISIS_PDU_OFFSET(pdu, ISIS_OFFSET_CSNP_LEN) = htobe16(pdu->pdu_len);
            break;
        case ISIS_PDU_L1_PSNP:
        case ISIS_PDU_L2_PSNP:
            pdu->tlv_offset = ISIS_HDR_LEN_COMMON+ISIS_HDR_LEN_PSNP;
            *ISIS_PDU_OFFSET(pdu, ISIS_OFFSET_HDR_LEN) = pdu->tlv_offset;
            *(uint16_t*)ISIS_PDU_OFFSET(pdu, ISIS_OFFSET_PSNP_LEN) = htobe16(pdu->pdu_len);
            break;
        case ISIS_PDU_L1_LSP:
        case ISIS_PDU_L2_LSP:
            pdu->tlv_offset = ISIS_HDR_LEN_COMMON+ISIS_HDR_LEN_LSP;
            *ISIS_PDU_OFFSET(pdu, ISIS_OFFSET_HDR_LEN) = pdu->tlv_offset;
            *(uint16_t*)ISIS_PDU_OFFSET(pdu, ISIS_OFFSET_LSP_LEN) = htobe16(pdu->pdu_len);
            break;
        default:
            break;
    }
...
isis_pdu_update_lifetime(isis_pdu_s *pdu, uint16_t lifetime)
{
    switch (pdu->pdu_type) {
        case ISIS_PDU_L1_LSP:
        case ISIS_PDU_L2_LSP:
            *(uint16_t*)ISIS_PDU_OFFSET(pdu, ISIS_OFFSET_LSP_LIFETIME) = htobe16(lifetime);
            break;
        default:
            break;
    }
...
isis_pdu_update_auth(isis_pdu_s *pdu, char *key)
{
    ...
    if(pdu->pdu_type == ISIS_PDU_L1_LSP || pdu->pdu_type == ISIS_PDU_L2_LSP) {
        ...
        lifetime = *(uint16_t*)ISIS_PDU_OFFSET(pdu, ISIS_OFFSET_LSP_LIFETIME);
        *(uint16_t*)ISIS_PDU_OFFSET(pdu, ISIS_OFFSET_LSP_LIFETIME) = 0;
        checksum = *(uint16_t*)ISIS_PDU_OFFSET(pdu, ISIS_OFFSET_LSP_CHECKSUM);
    ...
    switch(pdu->auth_type) {
        case ISIS_AUTH_HMAC_MD5:
            ...
            memset(ISIS_PDU_OFFSET(pdu, pdu->auth_data_offset), 0x0, ISIS_MD5_DIGEST_LEN);
            HMAC_CTX *hmac = HMAC_CTX_new();
            HMAC_Init_ex(hmac, key, strlen(key), EVP_md5(), NULL);
            HMAC_Update(hmac, pdu->pdu, pdu->pdu_len);
            HMAC_Final(hmac, ISIS_PDU_OFFSET(pdu, pdu->auth_data_offset), NULL);
            HMAC_CTX_free(hmac);
            break;
        default:
            break;
    ...
    if(pdu->pdu_type == ISIS_PDU_L1_LSP || pdu->pdu_type == ISIS_PDU_L2_LSP) {
        ...
        *(uint16_t*)ISIS_PDU_OFFSET(pdu, ISIS_OFFSET_LSP_LIFETIME) = lifetime;
        *(uint16_t*)ISIS_PDU_OFFSET(pdu, ISIS_OFFSET_LSP_CHECKSUM) = checksum;
    }
}
        ...
        case ISIS_PDU_L2_LSP:
            ...
            if(checksum_orig > 0) {
                checksum = calculate_fletcher_checksum(
                    ...
                    ISIS_OFFSET_LSP_CHECKSUM-ISIS_OFFSET_LSP_ID, 
                    pdu->pdu_len-ISIS_OFFSET_LSP_ID);
                *(uint16_t*)ISIS_PDU_OFFSET(pdu, ISIS_OFFSET_LSP_CHECKSUM) = checksum_orig;
...
isis_pdu_validate_auth(isis_pdu_s *pdu, isis_auth_type auth, char *key)
{
    ...
    switch (pdu->auth_type) {
        case ISIS_AUTH_CLEARTEXT:
            if(strncmp((char*)ISIS_PDU_OFFSET(pdu, pdu->auth_data_offset), key, pdu->auth_data_len) == 0) {
                return true;
            }
            break;
        case ISIS_AUTH_HMAC_MD5:
            ...
            memcpy(auth_data, ISIS_PDU_OFFSET(pdu, pdu->auth_data_offset), ISIS_MD5_DIGEST_LEN);
            isis_pdu_update_auth(pdu, key);
            if(memcmp(ISIS_PDU_OFFSET(pdu, pdu->auth_data_offset), auth_data, ISIS_MD5_DIGEST_LEN) == 0) {
                return true;
            } else {
                /* Restore wrong key. */
                memcpy(ISIS_PDU_OFFSET(pdu, pdu->auth_data_offset), auth_data, ISIS_MD5_DIGEST_LEN);
            }
            break;
        default:
            break;
...
{
    *ISIS_PDU_CURSOR(pdu) = value;
    ISIS_PDU_BUMP_WRITE_BUFFER(pdu, sizeof(uint8_t));
}

void
isis_pdu_add_u16(isis_pdu_s *pdu, uint16_t value)
{
    *(uint16_t*)ISIS_PDU_CURSOR(pdu) = htobe16(value);
    ISIS_PDU_BUMP_WRITE_BUFFER(pdu, sizeof(uint16_t));
}

void
isis_pdu_add_u32(isis_pdu_s *pdu, uint32_t value)
{
    *(uint32_t*)ISIS_PDU_CURSOR(pdu) = htobe32(value);
    ISIS_PDU_BUMP_WRITE_BUFFER(pdu, sizeof(uint32_t));
}

void
isis_pdu_add_u64(isis_pdu_s *pdu, uint64_t value)
{
    *(uint64_t*)ISIS_PDU_CURSOR(pdu) = htobe64(value);
    ISIS_PDU_BUMP_WRITE_BUFFER(pdu, sizeof(uint64_t));
}

void
isis_pdu_add_bytes(isis_pdu_s *pdu, uint8_t *buf, uint16_t len)
{
    memcpy(ISIS_PDU_CURSOR(pdu), buf, len);
    ISIS_PDU_BUMP_WRITE_BUFFER(pdu, len);
}
...
isis_pdu_add_tlv_p2p_adjacency_state(isis_pdu_s *pdu, uint8_t state, uint32_t vlindex)
{
    ...
    tlv->len = sizeof(state) + sizeof(vlindex);
    *tlv->value = state;
    *(uint32_t*)(&tlv->value[1]) = htobe32(vlindex);
    ISIS_PDU_BUMP_WRITE_BUFFER(pdu, sizeof(isis_tlv_s)+tlv->len);
}
...
isis_pdu_add_tlv_is_neighbor(isis_pdu_s *pdu, isis_adjacency_s *adjacency) 
{
    ...
    isis_tlv_s *tlv = (isis_tlv_s *)ISIS_PDU_CURSOR(pdu);
    uint8_t *tlv_cur = tlv->value;
    tlv->type = ISIS_TLV_IS_NEIGHBOR;
    tlv->len = 0;
    while(peer) {
        if(peer->state != ISIS_PEER_STATE_DOWN) {
            memcpy(tlv_cur, peer->mac, ETH_ADDR_LEN);
            tlv_cur += ETH_ADDR_LEN;
            tlv->len += ETH_ADDR_LEN;
        }
        peer = peer->next;
    }
    if(tlv->len) {
...
isis_pdu_padding(isis_pdu_s *pdu)
{
    ...
    memset(ISIS_PDU_CURSOR(pdu), 0x0, remaining);
