    ...
    json_object_foreach(config, key, value) {
        if (!strcmp(key, "access-line-profile-id")) {
            ...
            if (number == 0 || number >= UINT16_MAX) {
                fprintf(stderr, "JSON config error: Invalid value for access-line-profiles->access-line-profile-id\n");
                return false;
            }
            profile->access_line_profile_id = number;
            continue;
        }
        ...
        /* If none of the above key/values have matched. */
        fprintf(stderr, "JSON config error: Incorrect attribute name '%s' in access-line-profiles\n", key);
        return false;
    }

    if (!profile->access_line_profile_id) {
    ...
    json_object_foreach(lag, key, value) {
        ...
        if (!strcmp(key, "lacp-system-priority")) {
            ...
            if (number >= UINT16_MAX) {
                fprintf(stderr, "JSON config error: Invalid value for lag->lacp-system-priority-id\n");
                return false;
            }
        ...
        /* If none of the above key/values have matched. */
        fprintf(stderr, "JSON config error: Incorrect attribute name '%s' in interface->lag\n", key);
        return false;
    }

    if (!lag_config->interface) {
    ...
    json_object_foreach(link, key, value) {
        ...
        if (!strcmp(key, "lag-interface")) {
            ...
            if (!lag_present(string)) {
                fprintf(stderr, "JSON config error: Missing configuration for links->lag-interface, LAG %s not defined\n", string);
                return false;
            }
        ...
        /* If none of the above key/values have matched. */
        fprintf(stderr, "JSON config error: Incorrect attribute name '%s' in interfaces->links\n", key);
        return false;

    }

    if (!link_config->interface) {
...
json_parse_network_interface(json_t *network_interface, bbl_network_config_s *network_config)
{
    ...
    json_object_foreach(network_interface, key, value) {
        ...
        if (!strcmp(key, "address")) {
            if (!scan_ipv4_prefix(json_string_value(value), &network_config->ip)) {
                fprintf(stderr, "JSON config error: Invalid value for interfaces->network->address\n");
                return false;
            }
            continue;
        }
        ...
        if (!strcmp(key, "gateway")) {
            if (!inet_pton(AF_INET, json_string_value(value), &ipv4)) {
                fprintf(stderr, "JSON config error: Invalid value for interfaces->network->gateway\n");
                return false;
            }
        ...
        if (!strcmp(key, "address-ipv6")) {
            if (!scan_ipv6_prefix(json_string_value(value), &network_config->ip6)) {
                fprintf(stderr, "JSON config error: Invalid value for interfaces->network->address-ipv6\n");
                return false;
            }
            continue;              
        }        
        ...
        if (!strcmp(key, "gateway-ipv6")) {
            if (!inet_pton(AF_INET6, json_string_value(value), &network_config->gateway6)) {
                fprintf(stderr, "JSON config error: Invalid value for interfaces->network->gateway-ipv6\n");
                return false;
            }
            continue;
        }
        ...
        if (!strcmp(key, "gateway-mac")) {
            ...
            if (sscanf(string, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx",
                ...
                fprintf(stderr, "JSON config error: Invalid value for interfaces->network->gateway-mac\n");
                return false;
            }    
            continue;
        }
        ...
        if (!strcmp(key, "mtu")) {
            if (json_is_number(value)) {
                ...
                if (number < 64 || number > 9000) {
                    fprintf(stderr, "JSON config error: Invalid value for interfaces->network->mtu\n");
                    return false;
                }
        ...
        if (!strcmp(key, "isis-level") && json_is_number(value)) {
            ...
            if (network_config->isis_level < 1 || network_config->isis_level > 3) {
                fprintf(stderr, "JSON config error: Invalid value for interfaces->network->isis-level (1-3)\n");
                return false;
            }
            continue;
        }
    ...
    if (!network_config->isis_instance_id) {
        network_config->isis_level = 0;
    }

    if (!network_config->interface) {
        fprintf(stderr, "JSON config error: Missing value for interfaces->network->interface\n");
        return false;
    }
...
json_parse_access_interface(json_t *access_interface, bbl_access_config_s *access_config)
{
    ...
    access_config->username = g_ctx->config.username;
    access_config->password = g_ctx->config.password;
    access_config->authentication_protocol = g_ctx->config.authentication_protocol;
    access_config->agent_circuit_id = g_ctx->config.agent_circuit_id;
    access_config->agent_remote_id = g_ctx->config.agent_remote_id;
    ...
    json_object_foreach(access_interface, key, value) {
        ...
        if (!strcmp(key, "network-interface") && json_is_string(value)) {
            if (access_config->a10nsp_interface) {
                fprintf(stderr, "JSON config error: You can't define interfaces->access->network-interface and a10nsp-interface\n");
                return false;
            }
        ...
        if (!strcmp(key, "a10nsp-interface") && json_is_string(value)) {
            if (access_config->network_interface) {
                fprintf(stderr, "JSON config error: You can't define interfaces->access->a10nsp-interface and network-interface\n");
                return false;
            }
        ...
        if (!strcmp(key, "type") && json_is_string(value)) {
            string = json_string_value(value);
            if (!strcmp(string, "pppoe")) {
                access_config->access_type = ACCESS_TYPE_PPPOE;
            } else if (!strcmp(string, "ipoe")) {
                ...
                fprintf(stderr, "JSON config error: Invalid value for interfaces->access->type\n");
                return false;
            }
            continue;
        }
        ...
        if (!strcmp(key, "vlan-mode")) {
            ...
            if (!strcmp(string, "1:1")) {
                ...
                fprintf(stderr, "JSON config error: Invalid value for interfaces->access->vlan-mode\n");
                return false;
            }
            continue;
        }
        ...
        if (!strcmp(key, "outer-vlan-min") && json_is_number(value)) {
            access_config->access_outer_vlan_min = (int)json_number_value(value) & 4095;
            continue;
        }

        if (!strcmp(key, "outer-vlan-max") && json_is_number(value)) {
            access_config->access_outer_vlan_max = (int)json_number_value(value) & 4095;
            continue;
        }

        if (!strcmp(key, "outer-vlan-step") && json_is_number(value)) {
        ...
        if (!strcmp(key, "inner-vlan-min") && json_is_number(value)) {
            access_config->access_inner_vlan_min = (int) json_number_value(value) & 4095;
            continue;
        }

        if (!strcmp(key, "inner-vlan-max") && json_is_number(value)) {
            access_config->access_inner_vlan_max = (int) json_number_value(value) & 4095;
            continue;
        }

        if (!strcmp(key, "inner-vlan-step") && json_is_number(value)) {
        ...
        if (!strcmp(key, "address") && json_is_string(value)) {
            string = json_string_value(value);
            if (!inet_pton(AF_INET, string, &ipv4)) {
                fprintf(stderr, "JSON config error: Invalid value for interfaces->access->address\n");
                return false;
            }
        ...
        if (!strcmp(key, "address-iter") && json_is_string(value)) {
            string = json_string_value(value);
            if (!inet_pton(AF_INET, string, &ipv4)) {
                fprintf(stderr, "JSON config error: Invalid value for interfaces->access->address-iter\n");
                return false;
            }
        ...
        if (!strcmp(key, "gateway") && json_is_string(value)) {
            string = json_string_value(value);
            if (!inet_pton(AF_INET, string, &ipv4)) {
                fprintf(stderr, "JSON config error: Invalid value for interfaces->access->gateway\n");
                return false;
            }
        ...
        if (!strcmp(key, "gateway-iter") && json_is_string(value)) {
            string = json_string_value(value);
            if (!inet_pton(AF_INET, string, &ipv4)) {
                fprintf(stderr, "JSON config error: Invalid value for interfaces->access->gateway-iter\n");
                return false;
            }
        ...
        /* Optionally overload some settings per access interface function! */
        ...
        if (!strcmp(key, "authentication-protocol") && json_is_string(value)) {
            string = json_string_value(value);
            if (!strcmp(string, "PAP")) {
                access_config->authentication_protocol = PROTOCOL_PAP;
            } else if (!strcmp(string, "CHAP")) {
                access_config->authentication_protocol = PROTOCOL_CHAP;
            } else {
                fprintf(stderr, "JSON config error: Invalid value for interfaces->access->authentication-protocol\n");
                return false;
            }
            continue;
        }
        ...
        if (!strcmp(key, "igmp-version") && json_is_number(value)) {
            ...
            if (access_config->igmp_version < 1 || access_config->igmp_version > 3) {
                fprintf(stderr, "JSON config error: Invalid value for interfaces->access->igmp-version (1-3)\n");
                return false;
            }
            continue;
        }
        ...
        if (!strcmp(key, "session-group-id") && json_is_number(value)) {
            ...
            if (access_config->session_group_id >= UINT16_MAX) {
                fprintf(stderr, "JSON config error: Invalid value for interfaces->access->session-group-id\n");
                return false;
            }
            continue;
        }
        ...
        if (!strcmp(key, "stream-group-id") && json_is_number(value)) {
            ...
            if (access_config->stream_group_id >= UINT16_MAX) {
                fprintf(stderr, "JSON config error: Invalid value for interfaces->access->stream-group-id\n");
                return false;
            }
            continue;
        }
        ...
        /* If none of the above key/values have matched. */
        fprintf(stderr, "JSON config error: Incorrect attribute name '%s' in interfaces->access\n", key);
        return false;
    }

    if (!access_config->interface) {
        fprintf(stderr, "JSON config error: Missing value for interfaces->access->interface\n");
        return false;
    }
    ...
    if (!access_config->cfm_ma_name && access_config->cfm_cc) {
        fprintf(stderr, "JSON config error: Invalid config for interfaces->access->cfm-cc (missing cfm-ma-name)\n");
        return false;
    }
    ...
    if (access_config->access_type == ACCESS_TYPE_PPPOE) {
        ...
        if (!access_config->ipcp_enable) {
            ...
            access_config->dhcp_enable = false;
        }
    ...
    json_object_foreach(a10nsp_interface, key, value) {
        ...
        if (!strcmp(key, "mac") && json_is_string(value)) {
            ...
            if (sscanf(string, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx",
                ...
                fprintf(stderr, "JSON config error: Invalid value for interfaces->a10nsp->mac\n");
                return false;
            }
            continue;
        }
        ...
        /* If none of the above key/values have matched. */
        fprintf(stderr, "JSON config error: Incorrect attribute name '%s' in interfaces->a10nsp\n", key);
        return false;
    }

    if (!a10nsp_config->interface) {
        fprintf(stderr, "JSON config error: Missing value for interfaces->a10nsp->interface\n");
        return false;
    }
...
{    
    ...
    if (!bgp_config->ipv4_peer_address) {
...
static bool
json_parse_isis_external_connection(json_t *external, isis_external_connection_s *connection)
{
    /* Iterate over every key-value pair of object, running the block of code that follows 
     * each time with the proper values set to variables key and value. */
    const char *key;
    json_t *value = NULL;
    json_object_foreach(external, key, value) {
        if (!strcmp(key, "system-id")  && json_is_string(value)) {
            if (!isis_str_to_system_id(json_string_value(value), connection->system_id)) {
        ...
        if (!strcmp(key, "l1-metric") && json_is_number(value)) {
            connection->level[ISIS_LEVEL_1_IDX].metric = json_number_value(value);
            continue;
        }

        if (!strcmp(key, "l2-metric") && json_is_number(value)) {
        ...
        /* If none of the above key/values have matched. */
        fprintf(stderr, "JSON config error: Incorrect attribute name '%s' in isis->external->connections\n", key);
        return false;
    }

    if (!connection->system_id) {
...
static bool
json_parse_isis_external(json_t *external, isis_config_s *isis_config)
{
    /* Helper variables. */
    double number;
    const char *string = NULL;
    ...
    json_t *value = NULL;
    json_object_foreach(external, key, value) {
        if (!strcmp(key, "mrt-file") && json_is_string(value)) {
            isis_config->external_mrt_file = strdup(json_string_value(value));
            continue;
        }

        if (!strcmp(key, "connections") && json_is_array(value)) {
            size = json_array_size(value);
            for(i = 0; i < size; i++) {
                if (connection) {
                ...
                if(!json_parse_isis_external_connection(json_array_get(value, i), connection)) {
                    return false;
                }
            }
            continue;
        ...
        /* If none of the above key/values have matched. */
        fprintf(stderr, "JSON config error: Incorrect attribute name '%s' in isis->external\n", key);
        return false;
    }
    return true;
}
...
json_parse_isis_config(json_t *isis, isis_config_s *isis_config)
{
    ...
    json_object_foreach(isis, key, value) {
        if (!strcmp(key, "instance-id")) {
            number = json_number_value(value);
            if (number < 1 || number > UINT16_MAX) {
                fprintf(stderr, "JSON config error: Invalid value for isis->instance-id\n");
                return false;
            }
            isis_config->id = number;
            continue;
        }

        if (!strcmp(key, "isis-level")) {
            number = json_number_value(value);
            if (number < 1 || number > 3) {
                fprintf(stderr, "JSON config error: Invalid value for isis->level (1-3)\n");
                return false;
            }
            isis_config->level = number;
            continue;
        }
        ...
        if (!strcmp(key, "hostname") && json_is_string(value)) {
            isis_config->hostname = strdup(json_string_value(value));
            continue;
        }

        if (!strcmp(key, "router-id") && json_is_string(value)) {
            isis_config->router_id_str = strdup(json_string_value(value));
            continue;
        }

        if (!strcmp(key, "system-id") && json_is_string(value)) {
            isis_config->system_id_str = strdup(json_string_value(value));
            continue;
        }
        ...
        if (!strcmp(key, "external") && json_is_object(value)) {
            if(!json_parse_isis_external(value, isis_config)) {
                return false;
            }
            continue;
        }
        ...
        /* If none of the above key/values have matched. */
        fprintf(stderr, "JSON config error: Incorrect attribute name '%s' in isis\n", key);
        return false;
    }
    ...
    if (!isis_config->area_count) {
        /* Set default area. */
        isis_config->area = calloc(1, sizeof(isis_area_s));
        isis_config->area_count = 1;
        isis_str_to_area(g_default_area, isis_config->area);
    }

    /* Level 1 */
    if (!isis_config->level1_key || isis_config->level1_auth == ISIS_AUTH_NONE) {
        isis_config->level1_auth = ISIS_AUTH_NONE;
        isis_config->level1_auth_hello = false;
        isis_config->level1_auth_psnp = false;
        isis_config->level1_auth_csnp = false;
    }

    /* Level 2 */
    if (!isis_config->level2_key || isis_config->level2_auth == ISIS_AUTH_NONE) {
        isis_config->level2_auth = ISIS_AUTH_NONE;
        isis_config->level2_auth_hello = false;
        isis_config->level2_auth_psnp = false;
        isis_config->level2_auth_csnp = false;
    }
    return true;
}
...
{    
    ...
    json_object_foreach(ldp, key, value) {
        if (!strcmp(key, "instance-id")) {
            number = json_number_value(value);
            if (number < 1 || number > UINT16_MAX) {
                fprintf(stderr, "JSON config error: Invalid value for ldp->instance-id\n");
                return false;
            }
            ldp_config->id = number;
            continue;
        }
        ...
        if (!strcmp(key, "hostname") && json_is_string(value)) {
            ldp_config->hostname = strdup(json_string_value(value));
            continue;
        }

        if (!strcmp(key, "lsr-id") && json_is_string(value)) {
            ldp_config->lsr_id_str = strdup(json_string_value(value));
            continue;
        }
    ...
    if (!ldp_config->ipv4_transport_address) {
...
json_parse_stream(json_t *stream, bbl_stream_config_s *stream_config)
{
    ...
    json_object_foreach(stream, key, value) {
        ...
        if (!strcmp(key, "type") && json_is_string(value)) {
            ...
            } else if (strcmp(string, "ipv6") == 0) {
                stream_config->type = BBL_SUB_TYPE_IPV6;
            } else if (strcmp(string, "ipv6pd") == 0) {
        ...
        if (!strcmp(key, "direction") && json_is_string(value)) {
            string = json_string_value(value);
            if (strcmp(string, "upstream") == 0) {
                stream_config->direction = BBL_DIRECTION_UP;
            } else if (strcmp(string, "downstream") == 0) {
        ...
        /* If none of the above key/values have matched. */
        fprintf(stderr, "JSON config error: Incorrect attribute name '%s' in streams\n", key);
        return false;
    }

    if (!stream_config->name) {
...
json_parse_sessions(json_t *sessions) 
{
    ...
    json_object_foreach(sessions, key, value) {
        ...
        /* If none of the above key/values have matched. */
        fprintf(stderr, "JSON config error: Incorrect attribute name '%s' in sessions\n", key);
        return false;
    }
    return true;
}
...
json_parse_ipoe(json_t *ipoe)
{
    ...
    json_object_foreach(ipoe, key, value) {
        ...
        /* If none of the above key/values have matched. */
        fprintf(stderr, "JSON config error: Incorrect attribute name '%s' in ipoe\n", key);
        return false;
    }
    return true;
}
...
json_parse_pppoe(json_t *pppoe)
{
    ...
    json_object_foreach(pppoe, key, value) {
        ...
        if (!strcmp(key, "vlan-priority") && json_is_number(value)){
            ...
            g_ctx->config.pppoe_vlan_priority = number;
            continue;
        }
...
json_parse_ppp_auth(json_t *ppp_auth)
{
    ...
    json_object_foreach(ppp_auth, key, value) {
        if (!strcmp(key, "username") && json_is_string(value)) {
            g_ctx->config.username = strdup(json_string_value(value));
            continue;
        }

        if (!strcmp(key, "password") && json_is_string(value)) {
            g_ctx->config.password = strdup(json_string_value(value));
            continue;
        }
        ...
        if (!strcmp(key, "protocol") && json_is_string(value)) {
            string = json_string_value(value);
            if (strcmp(string, "PAP") == 0) {
                g_ctx->config.authentication_protocol = PROTOCOL_PAP;
            } else if (strcmp(string, "CHAP") == 0) {
        ...
        /* If none of the above key/values have matched. */
        fprintf( stderr, "JSON config error: Incorrect attribute name '%s' in ppp->authentication\n", key);
        return false;
    }
    return true;
}
...
json_parse_ppp_lcp(json_t *ppp_lcp)
{
    ...
    json_object_foreach(ppp_lcp, key, value) {
        ...
        /* If none of the above key/values have matched. */
        fprintf(stderr, "JSON config error: Incorrect attribute name '%s' in ppp->lcp\n", key);
        return false;
    }
    return true;
}
...
json_parse_ppp_ipcp(json_t *ppp_ipcp)
{
    ...
    json_object_foreach(ppp_ipcp, key, value) {
        ...
        /* If none of the above key/values have matched. */
        fprintf(stderr, "JSON config error: Incorrect attribute name '%s' in ppp->ipcp\n", key);
        return false;
    }
    return true;
}
...
json_parse_ppp_ip6cp(json_t *ppp_ip6cp)
{
    ...
    json_object_foreach(ppp_ip6cp, key, value) {
        ...
        /* If none of the above key/values have matched. */
        fprintf(stderr, "JSON config error: Incorrect attribute name '%s' in ppp->ip6cp\n", key);
        return false;
    }
    return true;
}
...
json_parse_ppp(json_t *ppp)
{
    ...
    json_object_foreach(ppp, key, value) {
        ...
        /* If none of the above key/values have matched. */
        fprintf(stderr, "JSON config error: Incorrect attribute name '%s' in ppp\n", key);
        return false;
    }
    return true;
}
...
json_parse_dhcp(json_t *dhcp)
{
    ...
    json_object_foreach(dhcp, key, value) {
        ...
        /* If none of the above key/values have matched. */
        fprintf(stderr, "JSON config error: Incorrect attribute name '%s' in dhcp\n", key);
        return false;
    }
    return true;
}
...
json_parse_dhcpv6(json_t *dhcpv6)
{
    ...
    json_object_foreach(dhcpv6, key, value) {
        ...
        if (!strcmp(key, "ldra") && json_is_boolean(value)){
        ...
        /* If none of the above key/values have matched. */
        fprintf(stderr, "JSON config error: Incorrect attribute name '%s' in dhcpv6\n", key);
    }
    return true;
}
...
json_parse_igmp(json_t *igmp)
{
    ...
    json_object_foreach(igmp, key, value) {
        if (!strcmp(key, "version")){
            ...
            g_ctx->config.igmp_version = number;
            continue;
        }
        ...
        if (!strcmp(key, "multicast-traffic-length") && json_is_number(value)){
            number = json_number_value(value);
            if (number < 76 || number > 1500) {
                fprintf(stderr, "JSON config error: Invalid value for igmp->multicast-traffic-length (must be between 76 and 1500)\n");
            }
            g_ctx->config.multicast_traffic_len = number;
            continue;
        }
        ...
        /* If none of the above key/values have matched. */
        fprintf(stderr, "JSON config error: Incorrect attribute name '%s' in igmp\n", key);
        return false;
    }
    return true;
}
...
json_parse_access_line(json_t *access_line)
{
    ...
    json_object_foreach(access_line, key, value) {
        if (!strcmp(key, "agent-circuit-id") && json_is_string(value)) {
            g_ctx->config.agent_circuit_id = strdup(json_string_value(value));
            continue;
        }

        if (!strcmp(key, "agent-remote-id") && json_is_string(value)) {
            g_ctx->config.agent_remote_id = strdup(json_string_value(value));
            continue;
        }
        ...
        /* If none of the above key/values have matched. */
        fprintf(stderr, "JSON config error: Incorrect attribute name '%s' in access-line\n", key);
        return false;
    }
    return true;
}
    ...
    json_object_foreach(traffic, key, value) {
        ...
        if (!strcmp(key, "max-burst") && json_is_number(value)){
            ...
            if (number < 1 || number > UINT8_MAX) {
                fprintf(stderr, "JSON config error: Invalid value for traffic->max-burst (must be between 1 and 255)\n");
                return false;
            }
        ...
        /* If none of the above key/values have matched. */
        fprintf(stderr, "JSON config error: Incorrect attribute name '%s' in traffic\n", key);
        return false;
    }
    return true;
}
...
json_parse_session_traffic(json_t *sess_traffic)
{
    ...
    json_object_foreach(sess_traffic, key, value) {
        ...
        /* If none of the above key/values have matched. */
        fprintf(stderr, "JSON config error: Incorrect attribute name '%s' in session-traffic\n", key);
        return false;
    }
    return true;
}
    ...
    json_object_foreach(interfaces, key, value) {
        if (!strcmp(key, "io-mode") && json_is_string(value)) {
            if (!strcmp(string, "packet_mmap_raw")) {
                ...
                io_packet_mmap_set_max_stream_len();
            } else if (strcmp(string, "packet_mmap") == 0) {
                g_ctx->config.io_mode = IO_MODE_PACKET_MMAP;
                io_packet_mmap_set_max_stream_len();
            } else if (strcmp(string, "raw") == 0) {
                g_ctx->config.io_mode = IO_MODE_RAW;
#if BNGBLASTER_DPDK
            } else if (strcmp(string, "dpdk") == 0) {
        ...
        if (!strcmp(key, "io-slots")){
            number = json_number_value(value);
            if (number < 32 || number >= UINT16_MAX) {
                fprintf(stderr, "JSON config error: Invalid value for interfaces->io-slots (must be between 32 and 65534)\n");
                return false;
            }
            g_ctx->config.io_slots = number;
            continue;
        }
        ...
        if (!strcmp(key, "mac-modifier")){
            number = json_number_value(value);
            if (number < 0 || number > UINT8_MAX) {
                fprintf(stderr, "JSON config error: Invalid value for interfaces->mac-modifier (must be between 32 and 255)\n");
                return false;
            }
            g_ctx->config.mac_modifier = number;
            continue;
        }
        ...
        /* If none of the above key/values have matched. */
        fprintf(stderr, "JSON config error: Incorrect attribute name '%s' in interfaces\n", key);
        return false;
    }

    if (g_ctx->config.io_mode == IO_MODE_PACKET_MMAP_RAW ||
        g_ctx->config.io_mode = IO_MODE_PACKET_MMAP) {
        io_packet_mmap_set_max_stream_len();
    }
...
bbl_config_streams_load_json(const char *filename)
{
    ...
    json_object_foreach(root, key, value) {
        ...
        /* If none of the above key/values have matched. */
        fprintf(stderr, "JSON stream config error: Incorrect top level attribute name '%s' in stream configuration\n", key);
        return false;
