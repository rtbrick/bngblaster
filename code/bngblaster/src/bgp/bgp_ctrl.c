...
bgp_ctrl_sessions(int fd, uint32_t session_id __attribute__((unused)), json_t *arguments)
{
    ...
    while(bgp_session) {
        ...
        if(bgp_session->ipv6_local_address && ipv6_addr_not_zero(&ipv6_local_address) && memcmp(bgp_session->ipv6_local_address, ipv6_local_address, sizeof(ipv6addr_t)) != 0) {
            bgp_session = bgp_session->next;
            continue;
        }
        if(bgp_session->ipv6_peer_address && ipv6_addr_not_zero(&ipv6_peer_address) && memcmp(bgp_session->ipv6_peer_address, ipv6_peer_address, sizeof(ipv6addr_t)) != 0) {
            bgp_session = bgp_session->next;
            continue;
        }
